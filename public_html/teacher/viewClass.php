...
<?php
    ...
    foreach (sql_to_array($conn,"SELECT `key` FROM `classes` WHERE `year`=".(date("m")>=9?date("Y"):date("Y")-1)." AND `teacherKey`=".$user["key"]." ORDER BY `key` DESC","key") as $irrelivant => $classKey) {
        ...
        foreach ($users as $keyU => $valueU) {
            $dataPoints[$valueU["email"]]["Student Name"]=$valueU["firstName"]." ".$valueU["lastName"];
            $dataPercent[$valueU["email"]]["Student Name"]=$valueU["firstName"]." ".$valueU["lastName"];
